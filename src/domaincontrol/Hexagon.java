    ...
    public void calculateBounds(Vector<Vector<String>> matrix) {
        ...
        for(int i = 0; i < lines; ++i) {
            ...
            for(int j = 0; j < columns; ++j) {
                ...
                if(x2 >= 0 && !(i%2 == 0) && !actual.equals(rightbound) && accesible(matrix.elementAt(x2/columns).elementAt(x2%columns))) aux.add(x2); //IF ITS NOT NEGATIVE, ITS NOT EVEN LINE AND IF ITS NOT RIGHTBOUND
                if(x3 >= 0 && !actual.equals(leftbound) && accesible(matrix.elementAt(x3/columns).elementAt(x3%columns))) aux.add(x3);
                if(x4 < total && !actual.equals(rightbound) && accesible(matrix.elementAt(x4/columns).elementAt(x4%columns))) aux.add(x4);
